...
for i=10,100 do
	surface.CreateFont("Font"..i, {font = "Arial", size = i, weight = 1000, antialias = true})
end

for i=10,100 do
	surface.CreateFont("FontO"..i, {font = "Open Sans", size = i, weight = 100, antialias = true})
end

for i=10,100 do
	surface.CreateFont("FontOB"..i, {font = "Open Sans", size = i, weight = 1000, antialias = true})
end

for i=10,100 do
	surface.CreateFont("FontA"..i, {font = "Arial", size = i, weight = 100, antialias = true})
end

for i=10,100 do
	surface.CreateFont("FontT"..i, {font = "Trebuchet24", size = i, weight = 0, antialias = true})
end
-- Color presets
local bg_colors = {
...
local function SpecHUDPaint(client)
   ...
   local time_y = round_y +3
	drawBlur( x , round_y, time_x - x - 40, height, 3, 6, 255 )
   draw.RoundedBox(0, x + 130, round_y, width - 130, height, Color(255,255,255))
   draw.RoundedBox(0, x , round_y, time_x - x - 40, height, Color(70,70,70,160))
	
   local text = L[ roundstate_string[GAMEMODE.round_state] ]
	draw.SimpleText( text, "FontOB26", x + margin, round_y + 2, Color(0,0,0))
   -- Draw round/prep/post time remaining
   local text = util.SimpleTime(math.max(0, GetGlobalFloat("ttt_round_end", 0) - CurTime()), "%02i:%02i")
   draw.SimpleText( text, "FontOB26",time_x + margin, time_y, Color(0,0,0))
...
local function InfoPaint(client)
   ...
   --DrawBg(x, y, width, height, client)
	...
	if client:Alive() then 
		if LocalPlayer():GetActiveWeapon():GetPrintName() != nil then 
			name = TryTranslation(LocalPlayer():GetActiveWeapon():GetPrintName() or LocalPlayer():GetActiveWeapon().PrintName or "...") 
		end
	end
	...
	if round_state == ROUND_ACTIVE then
		...
		elseif client:GetRole() == 2 then
			color = Color(0,0,255,100)
		elseif client:GetRole() == 0 then
			color = Color(0,255,0,100)
		else
			color = Color(255,70,70,100)
		end
	else
		color = Color(70,70,70,100)
	end

	surface.SetFont("FontO40")
	local namesize = surface.GetTextSize(string.upper(name))
	
	drawBlur( x, scrh , 60 + 40 + namesize, 60, 3, 6, 255 )
	draw.RoundedBox(0,70,scrh,40 + namesize,60,Color(255,255,255,255))
	draw.RoundedBox(0,10,scrh,60,60,color)
	draw.RoundedBox(0,70,scrh,1,60,Color(0,0,0,255))
   ...
   local healthbar = health
   if healthbar >= 100 then healthbar = 100 end
   
	

   --PaintBar(x + margin, health_y, bar_width, bar_height, health_colors, health/100)
	draw.RoundedBox(0,70,scrh - 40,180,30,Color(255,255,255,255))
	draw.RoundedBox(0,10,scrh - 40,60,30,Color(70,70,70,160))

	--draw.RoundedBox(0,140,scrh - 40,40,30,Color(200,200,200,255))
	
	draw.RoundedBox(0,70,scrh - 40,healthbar * 1.9,30,Color(255,80,80,255))
	
	draw.RoundedBox(0,70,scrh - 40,1,30,Color(0,0,0,255))
	
   --ShadowedText(tostring(health), "HealthAmmo", bar_width, health_y, COLOR_WHITE, TEXT_ALIGN_RIGHT, TEXT_ALIGN_RIGHT)
	
	surface.SetMaterial( Material("materials/core/hp.png") )
	surface.SetDrawColor(255, 255, 255, 255)
	surface.DrawTexturedRect( 25, scrh - 41, 32, 32 )
	
	draw.SimpleTextOutlined( health.."%", "FontOB36",160, scrh - 44, Color(255,255,255),1,0,1,Color(0,0,0,255))
	
	
	
	draw.SimpleText( string.upper(name), "FontO40",90, scrh + 7.5 , Color(0,0,0))
   ...
   if client:GetActiveWeapon().Primary then
      ...
      if ammo_clip != -1 then
		...
		draw.SimpleText( text, "FontA20",40 + namesize, scrh + 40, Color(0,0,0))
		--ShadowedText(text, "HealthAmmo", bar_width, ammo_y, COLOR_WHITE, TEXT_ALIGN_RIGHT, TEXT_ALIGN_RIGHT)
      else
		draw.SimpleText( "∞", "FontO30",80 + namesize, scrh + 35, Color(0,0,0))
	  end
   end
   ...
   if round_state == ROUND_ACTIVE then
	...
	-- role
	local lefttext = string.Left(text,1)
	local textx = 20
	if lefttext == "I" then textx = 30 end
	
	draw.SimpleText( lefttext, "FontA70",textx, scrh - 3, Color(255,255,255),0,0)
